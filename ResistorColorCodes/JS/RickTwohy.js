...
var resistorValues = {
  ...
  function decodeResistorColors(bands) {
    var bandArr = bands.split(' ');
    
    var baseNumString = '';
    var multiplierString = '';
    var toleranceString = '';
    for(var i = 0; i < bands.length; i++) {
      var val = resistorValues[bandArr[i]];
      if(i < 2) {
        baseNumString +=  val === undefined ? '' : val;
      }else if(i === 2){
        multiplierString = val === undefined ? '' : val;
      }else if(i === 3) {
        val = toleranceValues[bandArr[i]];
        val = val === undefined ? 20 : val;
        toleranceString = val;
      }
    }
    
    var multiplier = Math.pow(10, parseInt(multiplierString));
    var baseNum = (parseInt(baseNumString) * multiplier)
    var suffix = '';
    var percent = toleranceString + '%';
    
    if(baseNum < 1000) {
      finalString = baseNum + ' ohms, ' + percent;
    }else if(baseNum >= 1000 && baseNum < 1000000) {
      baseNum = baseNum / 1000
      suffix = 'k';
      finalString = baseNum + suffix + ' ohms, ' + percent
    }else if(baseNum >= 1000000) {
      baseNum = baseNum / 1000000
      suffix = 'M';
      finalString = baseNum + suffix + ' ohms, ' + percent
    }
  
    return baseNum + suffix + ' ohms, ' + percent;
  }